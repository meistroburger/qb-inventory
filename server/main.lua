		...
		if QBCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if QBCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				...
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					...
					TriggerClientEvent('QBCore:Notify', src, itemInfo["label"] .. " bought!", "primary")
		...
		if amount > 0 then
			if itemData ~= nil then
				...
				if Player.Functions.AddItem(itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, "You\'ve given [" ..GetPlayerName(tonumber(args[1])).."] | Item:["..itemData["name"].. "] | Label:["..itemData["label"].. "] | Amount:["..amount.."] | Weight:["..itemData["weight"].. "] | Type:["..itemData["type"].. "] | Image:["..itemData["image"].. "] | Description:["..itemData["description"].. "]", "primary")
					TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items[itemData["name"]], 'add')
				else
					TriggerClientEvent('QBCore:Notify', source,  "Can't give item!", "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source,  "Item doesn\'t not exist", "error")
			end
		else
			TriggerClientEvent('QBCore:Notify', source,  "Invalid item amount", "error")
		end
	else
...
QBCore.Commands.Add("randomitems10", "Give 10 random items (God Only)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local filteredItems = {}
...
QBCore.Commands.Add("randomitems20", "Give 20 random items (God Only)", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	local filteredItems = {}
	for k, v in pairs(QBCore.Shared.Items) do
		if QBCore.Shared.Items[k]["type"] ~= "weapon" then
