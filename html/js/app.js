            ...
            if ((ItemData.name).split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    // if (ItemData.info.attachments !== null && ItemData.info.attachments !== undefined && ItemData.info.attachments.length > 0) {
                        $(".inv-options-list").append('<center><div style="background-color: #30475e;" class="inv-option-item" id="weapon-attachments"><p>ATTACHMENTS</p></div></center>');
...
$(document).on('click', '.weapon-attachments-back', function(e){
    e.preventDefault();
    $("#qb-inventory").css({"display":"block"});
    $("#qb-inventory").animate({
        left: 0+"vw"
    }, 200);
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-title").html('<p>'+itemData.label+'</p>')
            $(".item-info-description").html('<p><strong>Serial Number: </strong><span>' + itemData.info.bagid);
            ...
            if (fromData.amount == $toAmount) {
                ...
                if ((newData.name).split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newData.name)) {
                        ...
                        var QualityColor = "rgb(48, 71, 94)";
                        if (newData.info.quality < 26) {
                            QualityColor = "rgb(242, 110, 101)";
                        } else if (newData.info.quality > 25 && newData.info.quality < 75) {
                            QualityColor = "rgb(143, 90, 97)";
                        } else if (newData.info.quality >= 75) {
                            QualityColor = "rgb(48, 71, 94)";
                        }
                    ...
                    if ((newDataFrom.name).split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(newDataFrom.name)) {
                            ...
                            if (newDataFrom.info.quality < 26) {
                                QualityColor = "rgb(242, 110, 101)";
                            } else if (newDataFrom.info.quality > 25 && newDataFrom.info.quality < 75) {
                                QualityColor = "rgb(143, 90, 97)";
                            } else if (newDataFrom.info.quality >= 75) {
                                QualityColor = "rgb(48, 71, 94)";
                            }
                ...
                if ((fromData.name).split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(fromData.name)) {
                        ...
                        if (fromData.info.quality < 26) {
                            QualityColor = "rgb(242, 110, 101)";
                        } else if (fromData.info.quality > 25 && fromData.info.quality < 75) {
                            QualityColor = "rgb(143, 90, 97)";
                        } else if (fromData.info.quality >= 75) {
                            QualityColor = "rgb(48, 71, 94)";
                        }
                    ...
                    if ((toData.name).split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(toData.name)) {
                            ...
                            if (toData.info.quality < 26) {
                                QualityColor = "rgb(242, 110, 101)";
                            } else if (toData.info.quality > 25 && toData.info.quality < 75) {
                                QualityColor = "rgb(143, 90, 97)";
                            } else if (toData.info.quality >= 75) {
                                QualityColor = "rgb(48, 71, 94)";
                            }
                ...
                if ((newDataTo.name).split("_")[0] == "weapon") {
                    if (!Inventory.IsWeaponBlocked(newDataTo.name)) {
                        ...
                        if (newDataTo.info.quality < 26) {
                            QualityColor = "rgb(242, 110, 101)";
                        } else if (newDataTo.info.quality > 25 && newDataTo.info.quality < 75) {
                            QualityColor = "rgb(143, 90, 97)";
                        } else if (newDataTo.info.quality >= 75) {
                            QualityColor = "rgb(48, 71, 94)";
                        }
                        if (newDataTo.info.quality !== undefined) {
        ...
        if (!Inventory.IsWeaponBlocked(item.name)) {
            if ((item.name).split("_")[0] == "weapon") {
                ...
                if (item.info.quality < 26) {
                    QualityColor = "rgb(242, 110, 101)";
                } else if (item.info.quality > 25 && item.info.quality < 75) {
                    QualityColor = "rgb(143, 90, 97)";
                } else if (item.info.quality >= 75) {
                    QualityColor = "rgb(48, 71, 94)";
                }
        ...
        window.addEventListener('message', function(event) {
            switch(event.data.action) {
                ...
                case "toggleHotbar":
                    ...
                    break;
                case "RobMoney":
                    $(".inv-options-list").append('<center><div style="background-color: #30475e;" class="inv-option-item" id="rob-money"><p>Take Cash</p></div>');
